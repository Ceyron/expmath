...
names = ["a", "a*t+b", "a*sin(b*t)", "a*cos(b*t)", ]


def calculate_new_value_pairs(x_active, x_a, x_b, y_active, y_a, y_b, t_start,
        t_end):
    t = np.linspace(t_start, t_end, NUM_T_POINTS)
    x = functions[x_active](x_a, x_b, t)
    y = functions[y_active](y_a, y_b, t)
    return x, y

def create_latex(x_active, x_a, x_b, y_active, y_a, y_b, t_start, t_end):
    ...
    y_function = (latex[y_active] % (y_a, y_b))[:-1]
    combined = """
        \\vec{\gamma}(t) = 
        \\begin{pmatrix}
            %s \cr
...
x_function_selector = RadioButtonGroup(name="Typ für x", labels=names, active=3)
y_function_selector = RadioButtonGroup(labels=names, active=2)
...
x_parameter_b = Slider(title="Parameter b für x", start=-2, end=2, step=0.1,
        value=1, visible=False)
y_parameter_a = Slider(title="Parameter a für y", start=-2, end=2, step=0.1,
        value=-1, visible=False)
y_parameter_b = Slider(title="Parameter b für y", start=-2, end=2, step=0.1,
...
def update_data(attr, old, new):
    x, y = calculate_new_value_pairs(x_function_selector.active,
            ...
            y_function_selector.active, y_parameter_a.value,
            y_parameter_b.value, parameter_t.value[0], parameter_t.value[1])
...
def toggle_callback(source):
    ...
    y_parameter_a.visible = True
    y_parameter_b.visible = True
...
for slider in (x_parameter_a, x_parameter_b, y_parameter_a, y_parameter_b,
        parameter_t):
...
inputs = WidgetBox(x_function_selector, y_function_selector, parameter_toggle,
        Row(Column(x_parameter_a, x_parameter_b),
            Column(y_parameter_a, y_parameter_b)), parameter_t)
